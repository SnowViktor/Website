function initializeSession() {
  ...
  console.log(
    ...
    sessionStorage.getItem("initialSessionPage")
  );
  console.log(
...
export function updateNavigationHistory() {
  ...
  if (
    ...
    sessionStorage.setItem("navigationHistory", JSON.stringify(historyArray));
    console.log("Navigation History (header - updated):", historyArray);
  } else {
    console.log("Navigation History (header - unchanged):", historyArray);
  }
}
...
export function navigateBack(a: HTMLAnchorElement) {
  ...
  a.addEventListener("click", (event) => {
    ...
    if (historyArrayOnClick.length > 1) {
      const previousPath = historyArrayOnClick[historyArrayOnClick.length - 2];
    ...
    if (shouldGoBack) {
      ...
      console.log(
        "Case 1: Allowed to website go back, executing history.back()"
      );
